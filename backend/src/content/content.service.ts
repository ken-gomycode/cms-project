...
export class ContentService {
   ...
   */
  async create(dto: CreateContentDto, authorId: string): Promise<ContentWithRelations> {
    ...
    if (dto.status === ContentStatus.SCHEDULED) {
      ...
      if (scheduledDate <= new Date()) {
        throw new BadRequestException('scheduledAt must be a future date when status is SCHEDULED');
      }
    }
    ...
    if (dto.featuredImageId) {
      ...
      if (!imageExists) {
        throw new NotFoundException(`Featured image with ID ${dto.featuredImageId} not found`);
      }
    }
    ...
    // Invalidate content list caches after creating new content
   ...
   */
  async findAll(dto: ContentFilterDto): Promise<PaginatedResponseDto<ContentWithRelations>> {
    ...
    return new PaginatedResponseDto(content as ContentWithRelations[], total, page, limit);
  }
   ...
   */
  async update(id: string, dto: UpdateContentDto, userId: string): Promise<ContentWithRelations> {
    ...
    // Invalidate content caches after update
    await this.invalidateContentCache(id, slug);
    await this.invalidateContentListCache();
    return content as ContentWithRelations;
  }
  ...
  async remove(id: string): Promise<void> {
    ...
    // Invalidate content caches after soft delete
    await this.invalidateContentCache(id, content.slug);
    await this.invalidateContentListCache();
  }
  ...
  async hardRemove(id: string): Promise<void> {
    ...
    // Invalidate content caches after hard delete
    await this.invalidateContentCache(id, content.slug);
    await this.invalidateContentListCache();
  }
   ...
   */
  async scheduleContent(id: string, scheduledAt: Date): Promise<ContentWithRelations> {
    // Check if content exists
    const content = await this.prisma.content.findUnique({
      where: { id },
    });
  ...
  async unscheduleContent(id: string): Promise<ContentWithRelations> {
    ...
    if (content.status !== ContentStatus.SCHEDULED) {
      throw new BadRequestException('Content is not scheduled and cannot be unscheduled');
    }
  ...
  /**
   * Invalidate specific content cache by ID and slug
   */
  private async invalidateContentCache(id: string, slug: string): Promise<void> {
    // Delete cache by ID (http:GET:/content/{id})
    await this.cacheService.del(`http:GET:/content/${id}`);

    // Delete cache by slug (http:GET:/content/{slug})
    await this.cacheService.del(`http:GET:/content/${slug}`);
  }

  /**
   * Invalidate content list cache
   * This should be called when any content is created, updated, or deleted
   */
  private async invalidateContentListCache(): Promise<void> {
    // Reset all list caches - in a production system you might want to
    // implement more granular cache invalidation
    // For now, we just delete the common list endpoint cache
